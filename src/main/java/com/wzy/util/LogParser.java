...
public class LogParser {
    ...
    final static Pattern table = Pattern.compile("业务\\[\\d+]"); // 匹配表名   // 匹配会员号
    ...
    public static void formatText(String line, HashMap<String, HashMap<String, String>> list, LogEntry totalEntry) {
        log.info(line);
        ...
        if (matcher.find()) {
            ...
            String memberId = matcher4.group();
            String txt2;
            String txt4;
            HashMap<String, String> memMap = list.get(memberId);
            if (memMap == null) {
                memMap = new HashMap<>();
                memMap.put(TableEnum.MEMBER.toString(), memberId);
            }
